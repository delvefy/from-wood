...
// interpolated evenly along the straight line between the anchors, so chains
// render as clean, evenly-spaced runs. Costs only set the resource mix — the
// global cost curve (index.ts) scales the amounts. Effects are uniform:
// every node gives the mode's flat gather AND craft bonus
// (NODE_BONUS_PERCENT in index.ts), regardless of branch.
...
// resource) and grants the standard per-mode gather/craft bonus.
